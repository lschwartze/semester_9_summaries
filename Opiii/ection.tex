...
\section{Lazy Random Walks in Algorithms}
	...
	\end{definition}
	\begin{definition}
		For a probability distribution $P_v^t$ of a random walk on $G$ starting in $v$ after $t$ rounds, we define $\tau_{mix}$ as follows the minimal $t$ such that \[\left|P_v^t(w) - \frac{1}{2md(u)}\right| \leq \frac{1}{2nmd(u)} \; \forall u,w \in V\]
	\end{definition}
