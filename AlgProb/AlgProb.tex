...
\begin{document}
	...
	Sei $G = (V,E)$ ein Graph mit $\left|V\right| = n$ gerade. Es sei weiter eine Totalordnung auf $V$ definiert.
	\begin{definition}
		Wir definieren die Matrix $A_G$ via \[(A_G)_{i,j} = \begin{cases}
			x_{i,j}, & (i,j) \in E \land i<j\\
			-x_{i,j}, & (i,j) \in E \land j<i\\
			0, & \text{ sonst}
		\end{cases}\]
	\end{definition}
	Bemerke, dass ein perfektes Matching in $G$ eine Permutation $\pi$ von $V$ ist, die aus disjunkten Zwei-Zyklen besteht, wobei $\pi(i) \neq i \; \forall i \in V$.
	\begin{lemma}
